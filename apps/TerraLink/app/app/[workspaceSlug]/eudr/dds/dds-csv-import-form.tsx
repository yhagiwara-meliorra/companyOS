...
import { importDdsCsv } from "@/lib/domain/eudr-actions";
...
const DDS_CSV_TEMPLATE =
  "internal_reference,operator_org_name,operator_type,country_of_activity,description\nDDS-2026-001,Meliorra Green Supply,operator,DE,Coffee import from Rwanda\n";

function downloadTemplate() {
  const bom = "\uFEFF";
  const blob = new Blob([bom + DDS_CSV_TEMPLATE], {
    type: "text/csv;charset=utf-8",
  });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "dds_template.csv";
  a.click();
  URL.revokeObjectURL(url);
}

export function DdsCsvImportForm({
  workspaceSlug,
}: {
  workspaceSlug: string;
}) {
  const [showDetail, setShowDetail] = useState(false);
  const boundAction = importDdsCsv.bind(null, workspaceSlug);
  ...
  return (
    <>
      {isDragging && <CsvDropOverlay label="DDS CSV をドロップしてインポート" />}
