...
export async function runStructuredStep<TSchema extends z.ZodTypeAny>(
  ...
  // Example direction:
  // - Claude: constitution_check, solution_frame, build_direction
  // - GPT: gtm_frame, risk_and_decision, artifact generation
  // Use each provider's structured-output JSON mode and parse with params.schema.
  return mockStructuredStep(params);
}
